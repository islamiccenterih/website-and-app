(() => {
  ...
  const dayIndex = (day) => Math.floor(Date.UTC(day.y, day.m - 1, day.d) / 86400000);
  const set = (sel, text) => {
  ...
  const paintHadith = (day) => {
    if (!hadith.length) return;
    const row = hadith[dayIndex(day) % hadith.length];
  ...
  const urStart = root.querySelector('[data-dq-ur]');
  showBox('[data-dq-ur-box]', !!(urStart && urStart.textContent.trim()));
  ...
  const load = (ayahNo, iso, i) => getJson('https://api.alquran.cloud/v1/ayah/' + ayahNo + '/editions/' + packs[i] + '?d=' + iso, 8000)
    .then((payload) => {
      ...
      set('[data-dq-ref]', ref);
      set('[data-dq-date]', 'India · ' + iso + ' · ayah ' + (ar.surah ? ar.surah.number + ':' + ar.numberInSurah : ayahNo));
      if (ar.surah) {
      ...
      if (i + 1 < packs.length) return load(ayahNo, iso, i + 1);
      throw new Error('empty');
    });

  const refresh = () => {
    const day = istDay();
    if (shownIso === day.iso && paintedLive) return;
    shownIso = day.iso;
    paintHadith(day);
    const ayahNo = (dayIndex(day) % 6236) + 1;
    load(ayahNo, day.iso, 0).catch(() => {
      if (!paintedLive) keepSaved();
    });
  };

  refresh();
  setInterval(() => {
    if (istDay().iso !== shownIso) {
      paintedLive = false;
      refresh();
    }
  }, 1000);
