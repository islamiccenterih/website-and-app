...
They can view their profile, **only their own published results**, and **Join class** for a live room belonging to **any course they are enrolled in**. Video, voice, and chat stay on this website (WebRTC). Join with mic only and turn the camera on later if you want. People and Chat open as separate side panels (like Google Meet). Use HTTPS or localhost so the browser allows camera and microphone.
